defmodule ProteinTranslation do
  ...
  defp translate_rna("", results), do: Enum.reverse(results)

  for codon <- @stop do
    defp translate_rna(unquote(codon) <> _rest, results), do: Enum.reverse(results)
    defp translate_codon(unquote(codon)), do: "STOP"
  end
  ...
  for { protein, codons } <- @codons, codon <- codons do
    ...
    defp translate_codon(unquote(codon)), do: unquote(protein)
  end
