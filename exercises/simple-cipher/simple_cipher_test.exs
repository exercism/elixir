...
defmodule SimpleCipherTest do
  ...
  describe "random key generation" do
    #@tag :pending
    test "random key is all lowercase letters" do
      key = SimpleCipher.key

      assert key != nil
      assert String.match?(key, ~r{\A[a-z]+\z}) == true
    end

    @tag :pending
    test "random key is 100 characters long" do
      key = SimpleCipher.key

      assert String.length(key) == 100
    end
  end
  ...
  describe "basic encoding/decoding" do
    ...
    @tag :pending
    test "decoding an encoded text with the same key returns original text" do
      key = "abc"
      plaintext = "this is a test"

      encoded = SimpleCipher.encode(plaintext, key)
      assert SimpleCipher.decode(encoded, key) == plaintext
    end
