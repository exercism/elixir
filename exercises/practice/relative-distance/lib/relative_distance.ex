defmodule RelativeDistance do
  ...
  def degree_of_separation(family_tree, person_a, person_b) do
    family_tree
    |> build_family_graph()
    |> find_degree(person_b, [{person_a, 0}], MapSet.new())
  end
  ...
  defp build_family_graph(family_tree) do
    for {parent, children} <- family_tree, child <- children, reduce: %{} do
      graph ->
        ...
        # sibling rule, inferred from "Sibling relationship"
        |> Map.update(child, siblings, &MapSet.union(&1, siblings))
    end
  end

  defp find_degree(_graph, _goal, [], _history), do: nil

  defp find_degree(_graph, goal, [{goal, count} | _], _history), do: count

  defp find_degree(graph, goal, [{person, count} | rest], history) do
    ...
    find_degree(graph, goal, rest ++ next_steps, history)
  end
end
