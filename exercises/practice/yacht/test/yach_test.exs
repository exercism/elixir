defmodule YachtTest do
  ...
  test "Full house two small, three big" do
    assert Yacht.score("full house", [2, 2, 4, 4, 4]) == 16
  end

  @tag :pending
  test "Full house three small, two big" do
    assert Yacht.score("full house", [5, 3, 3, 5, 3]) == 19
  end

  @tag :pending
  test "Two pair is not a full house" do
    assert Yacht.score("full house", [2, 2, 4, 4, 5]) == 0
  end

  @tag :pending
  test "Four of a kind is not a full house" do
    assert Yacht.score("full house", [1, 4, 4, 4, 4]) == 0
  end

  @tag :pending
  test "Yacht is not a full house" do
    assert Yacht.score("full house", [2, 2, 2, 2, 2]) == 0
  end

  @tag :pending
  test "Four of a Kind" do
    assert Yacht.score("four of a kind", [6, 6, 4, 6, 6]) == 24
  end

  @tag :pending
  test "Yacht can be scored as Four of a Kind" do
    assert Yacht.score("four of a kind", [3, 3, 3, 3, 3]) == 12
  end

  @tag :pending
  test "Full house is not Four of a Kind" do
    assert Yacht.score("four of a kind", [3, 3, 3, 5, 5]) == 0
  end

  @tag :pending
  test "Little Straight" do
    assert Yacht.score("little straight", [3, 5, 4, 1, 2]) == 30
  end

  @tag :pending
  test "Little Straight as Big Straight" do
    assert Yacht.score("big straight", [1, 2, 3, 4, 5]) == 0
  end

  @tag :pending
  test "Four in order but not a little straight" do
    assert Yacht.score("little straight", [1, 1, 2, 3, 4]) == 0
  end

  @tag :pending
  test "No pairs but not a little straight" do
    assert Yacht.score("little straight", [1, 2, 3, 4, 6]) == 0
  end

  @tag :pending
  test "Minimum is 1, maximum is 5, but not a little straight" do
    assert Yacht.score("little straight", [1, 1, 3, 4, 5]) == 0
  end

  @tag :pending
  test "Big Straight" do
    assert Yacht.score("big straight", [4, 6, 2, 5, 3]) == 30
  end

  @tag :pending
  test "Big Straight as little straight" do
    assert Yacht.score("little straight", [6, 5, 4, 3, 2]) == 0
  end

  @tag :pending
  test "No pairs but not a big straight" do
    assert Yacht.score("big straight", [6, 5, 4, 3, 1]) == 0
  end
